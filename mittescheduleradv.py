...
# Configure logging
logging.basicConfig(filename='uscmittes_execution.log', level=logging.INFO, format='%(asctime)s - %(message)s')
...
def job():
    ...
    logging.info(f'Finished uscmitte.py at {datetime.now()}')

# Define the start time as 20:00 on the current day (zou can replace anything in24 hour format )
start_time = datetime.combine(datetime.now(), datetime.strptime('00:23', '%H:%M').time())
