...
# Configure logging
logging.basicConfig(filename='uscmitte_execution.log', level=logging.INFO, format='%(asctime)s - %(message)s')

def job():
    logging.info(f'Starting uscmitte.py at {datetime.now()}')
    subprocess.call(['python', 'uscmitte.py']) # replace the script with your own gemini script
    logging.info(f'Finished uscbasic.py at {datetime.now()}')

# Define the start time as 20:00 on the current day (zou can replace anything in24 hour format )
start_time = datetime.combine(datetime.now(), datetime.strptime('23:30', '%H:%M').time())
logging.info(f'Start time: {start_time}')

# Calculate the target time as 8 hours after the start time
target_time = start_time + timedelta(hours=8) #Running from the start time plus 8 hours
